...
TEST_NAMSEOKCOIN_BINARY="src/test/test_namseokcoin"
...
if [[ ! -e ${TEST_NAMSEOKCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_NAMSEOKCOIN_BINARY} not found. Run \"cmake -B build -DCMAKE_BUILD_TYPE=Coverage\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_NAMSEOKCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
