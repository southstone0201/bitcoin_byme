# Bootstrappable Namseokcoin Core Builds

This directory contains the files necessary to perform bootstrappable Namseokcoin
Core builds.
...
To build Namseokcoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Namseokcoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/namseokcoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/namseokcoin-core/guix.sigs).
