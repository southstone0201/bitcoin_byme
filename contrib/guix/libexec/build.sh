#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Namseokcoin Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/namseokcoin-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Namseokcoin Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Namseokcoin Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    case "$HOST" in
        *darwin*)
            ...
            mv build/dist/Namseokcoin-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
    ...
    (
        ...
        # copy over the example namseokcoin.conf file. if contrib/devtools/gen-namseokcoin-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/namseokcoin.conf" "${DISTNAME}/"
