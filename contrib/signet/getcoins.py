#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Namseokcoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing namseokcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='namseokcoin-cli', help='namseokcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Namseokcoin address to which the faucet should send')
...
parser.add_argument('namseokcoin_cli_args', nargs='*', help='Arguments to pass on to namseokcoin-cli (default: -signet)')

args = parser.parse_args()

if args.namseokcoin_cli_args == []:
    args.namseokcoin_cli_args = ['-signet']


def namseokcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.namseokcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = namseokcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = namseokcoin_cli(['getnewaddress', 'faucet', 'bech32'])
