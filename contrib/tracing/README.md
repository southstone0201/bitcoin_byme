...
Tracing (USDT) support for Namseokcoin Core on Linux using. For more information on
USDT support in Namseokcoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `namseokcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `namseokcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Namseokcoin Core USDT documentation] on how to list available tracepoints in your
`namseokcoind` binary.

[Namseokcoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./build/src/namseokcoind
```
...
Namseokcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./build/src/namseokcoind
```
...
In a different terminal, starting Namseokcoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/namseokcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./build/src/namseokcoind
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./build/src/namseokcoind
```
