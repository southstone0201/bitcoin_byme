...
""" Example logging Namseokcoin Core mempool events using the mempool:added,
...
def main(namseokcoind_path):
    namseokcoind_with_usdts = USDT(path=str(namseokcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    namseokcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    namseokcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    namseokcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    namseokcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[namseokcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/namseokcoind")
        exit(1)
