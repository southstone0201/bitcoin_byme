...
"""Script for verifying Namseokcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from namseokcoincore.org and namseokcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/namseokcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://namseokcoincore.org"
HOST2 = "https://namseokcoin.org"
VERSIONPREFIX = "namseokcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Namseokcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"namseokcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by namseokcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the namseokcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes namseokcoin.org lags behind namseokcoincore.org.)')
    )
