UNIX BUILD NOTES
====================
Some notes on how to build Namseokcoin Core in Unix.
...
memory available when compiling Namseokcoin Core. On systems with less, gcc can be
...
To build Namseokcoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/namseokcoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/namseokcoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Namseokcoin Core can
