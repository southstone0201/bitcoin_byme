...
Below are some notes on how to build Namseokcoin Core for Windows.

The options known to work for building Namseokcoin Core on Windows are:
...
Note that for WSL the Namseokcoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/namseokcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\namseokcoin`, for example:

    cmake --install build --prefix /mnt/c/workspace/namseokcoin
