# CJDNS support in Namseokcoin Core

It is possible to run Namseokcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Namseokcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Namseokcoin network and individual nodes.
...
before it will work with your Namseokcoin Core node. This is described in steps
...
## Run Namseokcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Namseokcoin Core
...
When enabled, this option tells Namseokcoin Core that it is running in an
...
IPv6 local network. This helps Namseokcoin Core perform better address management:
...
CJDNS support was added to Namseokcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `namseokcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/namseokcoin/namseokcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Namseokcoin Core

There are several ways to see your CJDNS address in Namseokcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `namseokcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `namseokcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `namseokcoin-cli help getnodeaddresses` for details.
