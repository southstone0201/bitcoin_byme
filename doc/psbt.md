# PSBT Howto for Namseokcoin Core

Since Namseokcoin Core 0.17, an RPC interface exists for Partially Signed Namseokcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/namseokcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Namseokcoin transactions that are not fully signed
...
#### Multisig with multiple Namseokcoin Core instances
...
Namseokcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `namseokcoin-cli` is used.
...
Later, when *V* NSC has been received on *Amulti*, and Bob and Carol want to
