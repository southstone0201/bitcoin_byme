Namseokcoin Core version 0.18.0 is now available from:

  <https://namseokcoincore.org/bin/namseokcoin-core-0.18.0/>
...
`/Applications/Namseokcoin-Qt` (on Mac) or `namseokcoind`/`namseokcoin-qt` (on
Linux).
...
recommended to use Namseokcoin Core on unsupported systems.

Namseokcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Namseokcoin Core does not yet change appearance when
...
`contrib/init/namseokcoind.service`, it has been changed to use
`/var/lib/namseokcoind` as the data directory instead of
`~namseokcoin/.namseokcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/namseokcoind` will
exist has enough space (check using `df -h /var/lib/namseokcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/namseokcoin/namseokcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://namseokcoincore.org/en/contact/) the Namseokcoin Core
...
The systemd init file (`contrib/init/namseokcoind.service`) has been changed
to use `/var/lib/namseokcoind` as the data directory instead of
`~namseokcoin/.namseokcoin`. This change makes Namseokcoin Core more consistent with
...
`datadir` option in `/etc/namseokcoin/namseokcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/namseokcoin/namseokcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/namseokcoin/namseokcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/namseokcoin/namseokcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/namseokcoin/namseokcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/namseokcoin/namseokcoin/blob/master/doc/namseokcoin-conf.md)
  about the `namseokcoin.conf` file describes how to use it to configure
  Namseokcoin Core.

- A new document introduces Namseokcoin Core's BIP174 [Partially-Signed
  Namseokcoin Transactions
  (PSBT)](https://github.com/namseokcoin/namseokcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/namseokcoin/namseokcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Namseokcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Namseokcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/namseokcoin/namseokcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/namseokcoin/namseokcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Namseokcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Namseokcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/namseokcoin/namseokcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/namseokcoin/namseokcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart namseokcoind with the `-deprecatedrpc=generate`
...
- A new `namseokcoin-wallet` tool is now distributed alongside Namseokcoin
...
This section describes planned changes to Namseokcoin Core that may affect
other Namseokcoin software and services.

- Since version 0.16.0, Namseokcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Namseokcoin Core 0.20 (expected about
  a year after 0.18), Namseokcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Namseokcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Namseokcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Namseokcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Namseokcoin Core
...
- A sub-project of Namseokcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Namseokcoin Core.  See their [project
  page](https://github.com/namseokcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Namseokcoin Core's own implementation, although entropy
  gathered by Namseokcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Namseokcoin Core a
...
- On macOS, Namseokcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `namseokcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libnamseokcoin_server.a` (jnewbery)
...
- #13926 [Tools] namseokcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like namseokcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `namseokcoin-qt -version` output formatting (hebasto)
...
- #11625 Add NamseokcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libnamseokcoin_server` (laanwj)
...
- #15154 Configure: namseokcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/namseokcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Namseokcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libnamseokcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in namseokcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Namseokcoin Core (Christewart)
...
- #12246 Bugfix: Only run namseokcoin-tx tests when namseokcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_namseokcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_namseokcoin` (MarcoFalke)
...
- #14186 namseokcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_namseokcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update namseokcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/namseokcoin-conf.md (hebasto)
...
- #15354 Add missing `namseokcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct namseokcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support namseokcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/namseokcoin/).
