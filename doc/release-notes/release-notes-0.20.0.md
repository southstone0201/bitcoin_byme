...
Namseokcoin Core version 0.20.0 is now available from:

  <https://namseokcoincore.org/bin/namseokcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `NAMSEOKCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Namseokcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Namseokcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Namseokcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Namseokcoin Core. Namseokcoin Core logs debug messages
...
- OpenSSL is no longer used by Namseokcoin Core.  (#17265)

- BIP70 support has been fully removed from Namseokcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/namseokcoin/namseokcoin/issues/16599),
  [PR #16702](https://github.com/namseokcoin/namseokcoin/pull/16702), and the
  `namseokcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Namseokcoin Core starts are now
...
- The "Start Namseokcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://namseokcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Namseokcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Namseokcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/namseokcoin-core/HWI) for
  signing.  Future versions of Namseokcoin Core should support a GUI option
...
- Namseokcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.namseokcoincore.org.  (#17596)
...
- The `namseokcoin-cli` utility used with the `-getinfo` parameter now
...
- The `namseokcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `namseokcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_namseokcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBNAMSEOKCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_namseokcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBNAMSEOKCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for namseokcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate namseokcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest namseokcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_namseokcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (namseokcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone namseokcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Namseokcoin Core files (hebasto)
...
- #17119 Fix broken namseokcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from namseokcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.namseokcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to namseokcoin-wallet (hel-o)
...
- #18341 Replace remaining literal NSC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On namseokcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from namseokcoin-cli -getinfo (malevolent)
