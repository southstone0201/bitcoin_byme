# User-space, Statically Defined Tracing (USDT) for Namseokcoin Core

Namseokcoin Core includes statically defined tracepoints to allow for more
...
**Note**: Namseokcoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Namseokcoin Core
...
Multiple tools can list the available tracepoints in a `namseokcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Namseokcoin Core, use `info probes` in `gdb`:

```
$ gdb ./build/src/namseokcoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /build/src/namseokcoind
stap net        outbound_message 0x0000000000107c05 /build/src/namseokcoind
stap validation block_connected  0x00000000002fb10c /build/src/namseokcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Namseokcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/src/namseokcoind | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/src/namseokcoind -v
