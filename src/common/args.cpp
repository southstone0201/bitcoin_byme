...
const char * const NAMSEOKCOIN_CONF_FILENAME = "namseokcoin.conf";
const char * const NAMSEOKCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //namseokcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", NAMSEOKCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Namseokcoin
    //   new: C:\Users\Username\AppData\Local\Namseokcoin
    // macOS: ~/Library/Application Support/Namseokcoin
    // Unix-like: ~/.namseokcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Namseokcoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Namseokcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Namseokcoin";
#else
    // Unix-like
    return pathRet / ".namseokcoin";
#endif
#endif
}
