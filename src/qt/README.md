This directory contains the source code for the Namseokcoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### namseokcoingui.(h/cpp)

- Represents the main window of the Namseokcoin UI.
...
- ClientModel is used by the main application `namseokcoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a namseokcoin:... link from a browser).
...
* UI elements like NamseokcoinAmountField, which inherit from QWidget.
* `namseokcoinstrings.cpp`: automatically generated
* `namseokcoinunits.(h/cpp)`: NSC / mNSC / etc. handling
* `callback.h`
...
**Note:** Do not change `local/namseokcoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `namseokcoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `namseokcoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `namseokcoin-qt`
9. You're all set! Start developing, building, and debugging the Namseokcoin Core GUI
