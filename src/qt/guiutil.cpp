...
#include <qt/namseokcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Namseokcoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new NamseokcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new NamseokcoinAddressCheckValidator(parent));
}
...
bool parseNamseokcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no namseokcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("namseokcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!NamseokcoinUnits::parse(NamseokcoinUnit::NSC, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseNamseokcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseNamseokcoinURI(uriInstance, out);
}

QString formatNamseokcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("namseokcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(NamseokcoinUnits::format(NamseokcoinUnit::NSC, info.amount, false, NamseokcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openNamseokcoinConf()
{
    ...
    /* Open namseokcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Namseokcoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Namseokcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Namseokcoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Namseokcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "namseokcoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("namseokcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a namseokcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Namseokcoin\n";
        else
            optionFile << strprintf("Name=Namseokcoin (%s)\n", ChainTypeToString(chain));
