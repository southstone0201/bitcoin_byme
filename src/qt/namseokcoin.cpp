...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<NamseokcoinUnit>("NamseokcoinUnit");
#else
    qRegisterMetaType<NamseokcoinUnit>("NamseokcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. namseokcoin_de.qm (shortcut "de" needs to be defined in namseokcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. namseokcoin_de_DE.qm (shortcut "de_DE" needs to be defined in namseokcoin.qrc)
...
static const char* qt_argv = "namseokcoin-qt";

NamseokcoinApplication::NamseokcoinApplication()
...
void NamseokcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the NamseokcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", NamseokcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void NamseokcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool NamseokcoinApplication::createOptionsModel(bool resetSettings)
{
...
void NamseokcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new NamseokcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &NamseokcoinGUI::quitRequested, this, &NamseokcoinApplication::requestShutdown);
...
void NamseokcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void NamseokcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool NamseokcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void NamseokcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &NamseokcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &NamseokcoinApplication::handleRunawayException);
    connect(this, &NamseokcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &NamseokcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void NamseokcoinApplication::parameterSetup()
{
...
void NamseokcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void NamseokcoinApplication::requestInitialize()
{
...
void NamseokcoinApplication::requestShutdown()
{
...
void NamseokcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // namseokcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NamseokcoinGUI::handlePaymentRequest);
            connect(window, &NamseokcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void NamseokcoinApplication::handleRunawayException(const QString &message)
{
...
void NamseokcoinApplication::handleNonFatalException(const QString& message)
{
...
WId NamseokcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool NamseokcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NamseokcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(namseokcoin);
    Q_INIT_RESOURCE(namseokcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(NAMSEOKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see namseokcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see namseokcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse namseokcoin.conf, determine network, switch to network specific
    ...
    // namseokcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
