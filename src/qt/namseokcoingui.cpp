...
const std::string NamseokcoinGUI::DEFAULT_UIPLATFORM =
...
NamseokcoinGUI::NamseokcoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &NamseokcoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &NamseokcoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &NamseokcoinGUI::showModalOverlay);
...
void NamseokcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Namseokcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and namseokcoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &NamseokcoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &NamseokcoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &NamseokcoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Namseokcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Namseokcoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Namseokcoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Namseokcoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a namseokcoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Namseokcoin command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &NamseokcoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &NamseokcoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &NamseokcoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &NamseokcoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &NamseokcoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &NamseokcoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &NamseokcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &NamseokcoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &NamseokcoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &NamseokcoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &NamseokcoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &NamseokcoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &NamseokcoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &NamseokcoinGUI::showDebugWindow);
}

void NamseokcoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void NamseokcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &NamseokcoinGUI::setCurrentWalletBySelectorIndex);
...
void NamseokcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &NamseokcoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &NamseokcoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &NamseokcoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &NamseokcoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void NamseokcoinGUI::enableHistoryAction(bool privacy)
{
...
void NamseokcoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &NamseokcoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &NamseokcoinGUI::removeWallet);
...
WalletController* NamseokcoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void NamseokcoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &NamseokcoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &NamseokcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &NamseokcoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &NamseokcoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &NamseokcoinGUI::incomingTransaction);
    connect(this, &NamseokcoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void NamseokcoinGUI::removeWallet(WalletModel* walletModel)
{
...
void NamseokcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void NamseokcoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void NamseokcoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void NamseokcoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void NamseokcoinGUI::createTrayIcon()
{
...
void NamseokcoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &NamseokcoinGUI::toggleHidden);
...
void NamseokcoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void NamseokcoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void NamseokcoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void NamseokcoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void NamseokcoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void NamseokcoinGUI::openClicked()
{
...
void NamseokcoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void NamseokcoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void NamseokcoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void NamseokcoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void NamseokcoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void NamseokcoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void NamseokcoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void NamseokcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Namseokcoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void NamseokcoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void NamseokcoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void NamseokcoinGUI::updateHeadersSyncProgressLabel()
{
...
void NamseokcoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void NamseokcoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &NamseokcoinGUI::quitRequested);
...
void NamseokcoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void NamseokcoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &NamseokcoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void NamseokcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void NamseokcoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &NamseokcoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &NamseokcoinGUI::show);
                e->ignore();
            }
        }
    }
...
void NamseokcoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void NamseokcoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void NamseokcoinGUI::incomingTransaction(const QString& date, NamseokcoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(NamseokcoinUnits::formatWithUnit(unit, amount, true));
...
void NamseokcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void NamseokcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool NamseokcoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool NamseokcoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void NamseokcoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void NamseokcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void NamseokcoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void NamseokcoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void NamseokcoinGUI::updateWindowTitle()
{
...
void NamseokcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void NamseokcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void NamseokcoinGUI::detectShutdown()
{
...
void NamseokcoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void NamseokcoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(NamseokcoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = NamseokcoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void NamseokcoinGUI::subscribeToCoreSignals()
{
...
void NamseokcoinGUI::unsubscribeFromCoreSignals()
{
...
bool NamseokcoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<NamseokcoinUnit> units = NamseokcoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const NamseokcoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, NamseokcoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const NamseokcoinUnit u : NamseokcoinUnits::availableUnits()) {
        menu->addAction(NamseokcoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(NamseokcoinUnit newUnits)
{
    setText(NamseokcoinUnits::longName(newUnits));
}
