...
/** Call node.updateRwSetting() with Namseokcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Namseokcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/namseokcoin/namseokcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because namseokcoin 22.x will not try to read these.
...
//! Convert enabled/size values to namseokcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from namseokcoin -prune setting.
...
//! Get pruning size value to show in GUI from namseokcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayNamseokcoinUnit")) {
        settings.setValue("DisplayNamseokcoinUnit", QVariant::fromValue(NamseokcoinUnit::NSC));
    }
    QVariant unit = settings.value("DisplayNamseokcoinUnit");
    if (unit.canConvert<NamseokcoinUnit>()) {
        m_display_namseokcoin_unit = unit.value<NamseokcoinUnit>();
    } else {
        m_display_namseokcoin_unit = NamseokcoinUnit::NSC;
        settings.setValue("DisplayNamseokcoinUnit", QVariant::fromValue(m_display_namseokcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if namseokcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_namseokcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<NamseokcoinUnit>() == m_display_namseokcoin_unit) return;
    m_display_namseokcoin_unit = new_unit.value<NamseokcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayNamseokcoinUnit", QVariant::fromValue(m_display_namseokcoin_unit));
    Q_EMIT displayUnitChanged(m_display_namseokcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/namseokcoin/namseokcoin/pull/8273
    ...
    // (https://github.com/namseokcoin-core/gui/issues/567).
