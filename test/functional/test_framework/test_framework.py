#!/usr/bin/env python3
# Copyright (c) 2014-present The Namseokcoin Core developers
...
TMPDIR_PREFIX = "namseokcoin_func_test_"
...
class NamseokcoinTestMetaClass(type):
    """Metaclass for NamseokcoinTestFramework.

    Ensures that any attempt to register a subclass of `NamseokcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'NamseokcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("NamseokcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("NamseokcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class NamseokcoinTestFramework(metaclass=NamseokcoinTestMetaClass):
    """Base class for a namseokcoin test script.

    Individual namseokcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave namseokcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop namseokcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use namseokcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "namseokcoind": ("namseokcoind", "NAMSEOKCOIND"),
            "namseokcoin-cli": ("namseokcoincli", "NAMSEOKCOINCLI"),
            "namseokcoin-util": ("namseokcoinutil", "NAMSEOKCOINUTIL"),
            "namseokcoin-wallet": ("namseokcoinwallet", "NAMSEOKCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: namseokcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("NamseokcoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'namseokcoind', self.options.namseokcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'namseokcoin-cli', self.options.namseokcoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                namseokcoind=binary[i],
                namseokcoin_cli=binary_cli[i],
                version=versions[i],
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a namseokcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple namseokcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the namseokcoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as namseokcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    namseokcoind=self.options.namseokcoind,
                    namseokcoin_cli=self.options.namseokcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in namseokcoin.conf
    ...
    def skip_if_no_namseokcoind_tracepoints(self):
        """Skip the running test if namseokcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("namseokcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_namseokcoind_zmq(self):
        """Skip the running test if namseokcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("namseokcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if namseokcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("namseokcoin-wallet has not been compiled")

    def skip_if_no_namseokcoin_util(self):
        """Skip the running test if namseokcoin-util has not been compiled."""
        if not self.is_namseokcoin_util_compiled():
            raise SkipTest("namseokcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if namseokcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("namseokcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether namseokcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether namseokcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_namseokcoin_util_compiled(self):
        """Checks whether namseokcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_NAMSEOKCOIN_UTIL")
