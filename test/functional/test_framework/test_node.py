...
"""Class for namseokcoind node under test"""
...
NAMSEOKCOIND_PROC_WAIT_TIMEOUT = 60
...
class TestNode():
    """A class for representing a namseokcoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, namseokcoind, namseokcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the namseokcoin.conf file.
        # This means that starting a namseokcoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(namseokcoin_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any namseokcoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then namseokcoind would bind P2P ports on
        ...
        # namseokcoin.conf. To avoid collisions on 127.0.0.1:18445, change it to
        # 127.0.0.1:tor_port().
        ...
        # Add a new stdout and stderr file each time namseokcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by namseokcoind, and
        ...
        self.running = True
        self.log.debug("namseokcoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the namseokcoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'namseokcoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; namseokcoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to namseokcoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; namseokcoind is still starting
    ...
    def wait_until_stopped(self, *, timeout=NAMSEOKCOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def replace_in_config(self, replacements):
        ...
        The substitutions are passed as a list of search-replace-tuples, e.g.
            [("old", "new"), ("foo", "bar"), ...]
        """
        with open(self.namseokcoinconf, 'r', encoding='utf8') as conf:
            conf_data = conf.read()
        for replacement in replacements:
            assert_equal(len(replacement), 2)
            old, new = replacement[0], replacement[1]
            conf_data = conf_data.replace(old, new)
        with open(self.namseokcoinconf, 'w', encoding='utf8') as conf:
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into namseokcoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to namseokcoind
        expected_msg: regex that stderr should match when namseokcoind fails

        Will throw if namseokcoind starts without an error.
        Will throw if an expected_msg is provided and it does not match namseokcoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'namseokcoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'namseokcoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    """Interface to namseokcoin-cli for an individual node"""
    def __init__(self, binary, datadir):
        self.options = []
        self.binary = binary
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.namseokcoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with namseokcoin-cli command-line options
    ...
    def send_cli(self, clicommand=None, *args, **kwargs):
        """Run namseokcoin-cli command. Deserializes returned string as python object."""
        ...
        self.log.debug("Running namseokcoin-cli {}".format(p_args[2:]))
